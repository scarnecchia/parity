...
def test_readme_workflow(fixtures: Path, tmp_path: Path) -> None:
    project = Path(__file__).resolve().parents[1]
    wheel_dir = tmp_path / "wheels"
    wheel_dir.mkdir()
    build = subprocess.run(
    ...
    python_exe = environment / "bin" / "python"
    wheel = next(wheel_dir.glob("*.whl"))
    install = subprocess.run(
        ...
        text=True,
        check=False,
        env={**__import__("os").environ, "TMPDIR": str(project / ".tmp")},
    )
    assert install.returncode == 0, install.stderr

    def staged_roots(name: str) -> tuple[Path, Path]:
        sas = tmp_path / name / "sas"
        python = tmp_path / name / "python"
        for root in (sas, python):
            (root / "dplocal").mkdir(parents=True)
            (root / "msoc").mkdir()
        sas_fixture = sas / "dplocal" / "products.sas7bdat"
        shutil.copyfile(fixtures / "productsales.sas7bdat", sas_fixture)
        polars_readstat.ScanReadstat(str(sas_fixture)).df.collect().write_parquet(
            python / "dplocal" / "products.parquet"
        )
        return sas, python

    for name in ("cli", "config"):
        sas, python = staged_roots(name)
        output = tmp_path / name / "report"
        command = [str(environment / "bin" / "parity"), "run"]
        if name == "cli":
            command.extend(
                ["--sas_root", str(sas), "--python_root", str(python), "--output-dir", str(output)]
            )
        else:
        ...
        detail_path = output / summary["detail_links"][summary["datasets"][0]["id"]]
        assert detail_path.is_file()
        assert len(detail_path.read_text().splitlines()) == 2880
