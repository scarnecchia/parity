...
`summary.json` includes the schema version, overall status, package versions, execution limits, dataset entries, preview truncation counts, and `detail_links` mapping dataset IDs to relative JSONL paths. Dataset entries include names, status, reason, row counts, `matched_pairs`, `sas_only`, `python_only`, and `detail_complete`. Available metadata includes reader schemas and metadata, original filenames, and original/normalized describe output for successfully compared pairs. Fields and metadata available for one-sided or error entries differ from successful pairs.
