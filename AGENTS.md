...
- Unmatched rows are paired by occurrence rank (`pair_id`); each detail record carries `differing_columns`. The report leads each failed dataset with a plain-language diagnosis (schema deltas list the differing columns per side; value mismatches name each differing column with its row-pair count) and renders only the differing columns of each pair side by side; schema mismatches render no value table. Pairs are diagnostics, not proof of row correspondence. Report dataset sections are ordered by dataset name. Comparison is order-independent; `row_order_mismatches` counts matched rows whose file positions differ, and the report flags it at the top of the dataset section. A completed comparison reports WARN instead of FAIL when every unmatched occurrence's `differing_columns` fall inside character-vs-numeric columns; those columns are listed in `type_mismatched_columns`, and WARN does not fail the run (exit 0).
