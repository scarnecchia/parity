...
- `io/` and `runner.py` are imperative shells for TOML/filesystem access, polars-readstat/Arrow staging, DuckDB comparison, and report output. Keep each runtime source file classified by its `# pattern:` line. `--id` (`id`) places the report in `output_dir`/`<id>` — letters, numbers, and underscores only, at most 64 characters, `details` reserved — the effective report directory must be absent or empty, and `summary.json` records the id as `request_id`. Every run that creates the report directory writes structured JSONL events to `run.jsonl` (`io/run_log.py`; `--verbose` echoes them to stderr): counts, durations, paths, and OSError errno/strerror only — never cell values or raw exception text. Failed runs keep `run.jsonl`, so the directory stays non-empty until manually cleared — never delete the log to make a retry pass. `--threads` (default 4) sets DuckDB worker threads; the underscore flag aliases `--sas_root`/`--python_root` are gone in favor of `-s`/`-p`.
